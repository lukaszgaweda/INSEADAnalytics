...
* A1. It can be inferred from the table above that sum of top 6 components explains 52.29% of the variance of the data 
* A2. Top 3 highest weights of the principal component portfolio are: 5 yr T-Notes US	(0.93); 10yr T-Notes (0.93) and Treasury Bonds	(0.83)
* A3. My suggestion of the components interpretation is:
  1: Bonds
  2: Fx
  3: European SE indexes
  4. US SE indexes
  5. Interbank rates/Europeand Bond market futures
  6. Oil and oil-related commodity 
  7. Metal commodities
  8. Crop-related commodities
  9. Precious-metal commodities
  10. Asian stock indexes
* A4. 
  
  We first define the precrisis and crisis data variables and later we plot 
...
PreCrisis_Variance_Explained_Table_results<-PCA(pre_crisis_data, graph=FALSE)
...
And the pre-crisis scree plot

```{r echo=TRUE}
eigenvalues  <- PreCrisis_Variance_Explained_Table[, "Eigenvalue"]
```
...
Crisis_Variance_Explained_Table<-PreCrisis_Variance_Explained_Table_results$eig
...
And the crisis-period scree plot

```{r echo=TRUE}
eigenvalues  <- Crisis_Variance_Explained_Table[, "Eigenvalue"]
```
...
I've computed the details of the pree-crisis and crisis-period components:

Pre-crisis:

```{r echo=TRUE, tidy=TRUE}
pre_crisis_corused = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
pre_crisis_Rotated_Results<-principal(pre_crisis_corused, nfactors=20, rotate="varimax",score=TRUE)
pre_crisis_Rotated_Factors<-round(pre_crisis_Rotated_Results$loadings,2)
pre_crisis_Rotated_Factors<-as.data.frame(unclass(pre_crisis_Rotated_Factors))
colnames(pre_crisis_Rotated_Factors)<-paste("comp",1:ncol(pre_crisis_Rotated_Factors),sep=" ")

sorted_rows <- sort(pre_crisis_Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
pre_crisis_Rotated_Factors <- pre_crisis_Rotated_Factors[sorted_rows,]
pre_crisis_Rotated_Factors[abs(pre_crisis_Rotated_Factors) < 0.3]<-NA
```

```{r}
show_data <- pre_crisis_Rotated_Factors 
iprint.df(show_data, scale=TRUE)
```
...
crisis_Rotated_Results<-principal(crisis_corused, nfactors=20, rotate="varimax",score=TRUE)
crisis_Rotated_Factors<-round(crisis_Rotated_Results$loadings,2)
crisis_Rotated_Factors<-as.data.frame(unclass(crisis_Rotated_Factors))
colnames(crisis_Rotated_Factors)<-paste("comp",1:ncol(crisis_Rotated_Factors),sep=" ")

sorted_rows <- sort(crisis_Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
crisis_Rotated_Factors <- crisis_Rotated_Factors[sorted_rows,]
crisis_Rotated_Factors[abs(crisis_Rotated_Factors) < 0.3]<-NA
```

```{r}
show_data <- crisis_Rotated_Factors 
iprint.df(show_data, scale=TRUE)
```
*
*It can be seen that although the components structure has not changed a lot, the priority/weight of their has significantly changed. For example the 3rd most important component has switched form European SE indexes to Crop comoddities. This would be explained by crisis increasing the variation of the returns on some of the futures. 
*
*
...
* A1. So the code first creates an empty table "signal_used" of the same dimensions as the data with returns of futures. Subsequently moving average (ma) is calculated across the returns data for periods of days determined by MAfreq variable (a sequence of numbers from 10 to 250 with 20 incrementals). If the average return for the period was positive, 1 is added to the return table. If negative, -1 is added. In the next step, the return table is normalized so the sum of all variables is equal to 1. The new values would represent our investment strategy, where we short sell the futures with negative sign (decreasing trend) and with the money we buy the futures with positive sign (increasing trend). The strategy requires only 1$ of our investment, since the short selled futures values covers the long positions. 

Of course, this strategy is based on the historical data, hence the actual strategy would need to re-use the signals with a 2 days shift (as you need to make the decision before the begining of the new day of the return). 

In the last lines, we calculate the potential returns of the strategy applied over the historical period.


A2. An example of an altered strategy, where I believe that the overall business cycles (that determined) are present only over a shorter period of time, but are much more frequent:

```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,100,by=10) #changed parameters
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
