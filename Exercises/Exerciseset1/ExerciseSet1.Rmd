...
<br>
<br>
I have found a nice Bloomberg package that allows to download the trading volumes of the stock.
<hr>
<hr>
...
**Your Answers here:**
<br>
1. it is sourced in the file "dataSet1.R" (line in code: r source("dataSet1.R"))
<br>
2. 
- cumulative retunrs from: sum(StockReturns[,2]) 
- average from: mean(StockReturns[,2])
- std from: sd(StockReturns[,2])

3. Yes, we need to change the sticker in the data importing script to YAhoo sticker
...
**Your Answers here:**
<br>
We set up a new data variable:
transposedData <- t(StockReturns)

We use nrow(transposedData) for number of rows and ncol(transposedData) for number of columns
...
**Your Answers here:**
<br>
1. In dataSet1.R file I have to change startDate to "2005-01-01" 
2. In dataSet1.R file I need to edit the myticker varbiable  
<br>
...
**Your Answers here:**
<br>
1. We try to compare whether the data after csv export-import has changed. In fact it did so - we can see that the improper use of the separator makes the overall data structure change
<br>
2. dont know :-(
<br>
<br>
